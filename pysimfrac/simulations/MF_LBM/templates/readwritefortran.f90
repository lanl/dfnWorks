...
!    OPEN(UNIT=9,FILE=trim(adjustl(wall_file_path)),STATUS='OLD',FORM='UNFORMATTED',access='stream')
    OPEN(UNIT=9,FILE=trim(adjustl(wall_file_path)),STATUS='OLD')
    read(9,*)nx0,ny0,nz0
    print*,'original porous media sample size: ', 'nx0=', nx0, 'ny0=', ny0,'nz0=', nz0
    allocate(walls_raw(1:nx0,1:ny0,1:nz0))
    read(9,*)(((walls_raw(k,i,j),k=1,nx0),i=1,ny0),j=1,nz0)
!     read(9,*) walls_raw
    close(9)
...
!    walls(:,ny,:)=1
    ...
    print*, 'final grid: nx= ', nx, ' ny= ', ny, ' nz= ', nz

    call visual
    !call visual_skip_half

    open(11, file='./output/walls_processed_long.dat', status='replace',form='unformatted',access='stream')
    write(11)nx,ny,nz
...
subroutine visual   !solid geometry
    ...
    open(unit=ivtk,file='./output/geometry.vtk',FORM='unformatted',access='stream',status='unknown',convert='BIG_ENDIAN')
