...
/*! The MAIN of dfnTrans code. Input argument is a control file with input parameters to dfnTrans.
 The main parts of particle tracking simulations are called:
...
int main (int argc, char* controlf[]) {
    ...
    if (inputfile.flag != 0) {
        ...
        seed = time(0);
        srand48(seed);
        printf("The random generator uses the value of seed %d \n", seed);
    }
