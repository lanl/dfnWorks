...
def mesh_network(jobname, num_frac, ncpu):
    '''
    ...
    ## input checking over
    num_poly, h, visual_mode, dudded_points, domain = mesh.parse_params_file()

    mesh.create_parameter_mlgi_file(num_poly, h)

    mesh.create_lagrit_scripts(visual_mode, ncpu)
    mesh.create_user_functions()

    failure = mesh.mesh_fractures_header(num_poly, ncpu, visual_mode)

    if failure:
    	mesh.cleanup_dir()
    	sys.exit("One or more fractures failed to mesh properly.\nExiting Program")

    n_jobs = mesh.create_merge_poly_files(ncpu, num_poly, visual_mode)

    mesh.merge_the_meshes(num_poly, ncpu, n_jobs, visual_mode)
