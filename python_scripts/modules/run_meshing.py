...
def mesh_worker(work_queue, done_queue, visual_mode, num_poly):
...
def mesh_fractures_header(num_poly, ncpu, visual_mode):
    ...
    for i in xrange(ncpu):
        p = mp.Process(target=mesh_worker, args=(work_queue, \
...
def merge_worker(job):
    ...
    cmd=cmd%(job,job)
    print("Starting Process %d "%job)
    os.system(cmd)
    print("Process %d finished"%job)
...
def merge_the_meshes(num_poly, ncpu, n_jobs, visual_mode):
    ...
    for i in xrange(n_jobs):
        p = mp.Process(target=merge_worker, args=(i+1,)) 
        p.daemon = True
        p.start()        
        processes.append(p)
