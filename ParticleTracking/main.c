...
/**** open control file to read main parameters **************************/   
   ...
   res=strncmp(inputfile.filename,"years",5);
   if (res==0)
     timeunit=365.0*24.0*60.0*60.0;
 
   else
   {
     res=strncmp(inputfile.filename,"days",4);
   if (res==0)
     timeunit=24.0*60.0*60.0;
    else
    { 
     res=strncmp(inputfile.filename,"hours",5);
   if (res==0)
     timeunit=60.0*60.0;
     else
     {
      res=strncmp(inputfile.filename,"minutes",7);
   if (res==0)
     timeunit=60.0;
     else
     {
      res=strncmp(inputfile.filename,"seconds",7);
   if (res==0)
     timeunit=1.0; 
     else
       timeunit=1.0;   
