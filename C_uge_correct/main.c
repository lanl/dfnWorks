...
void readInParams(FILE *fparams, Params *params){
    ...
    // Read in Params File
    // line 1
    if(fscanf(fparams, "%s", params->mesh_file) != 1){printf("ERROR reading in mesh file name\n");}
    // line 2
    if(fscanf(fparams, "%s", &params->matID_file) != 1){printf("ERROR reading in matID file name\n");}
    // line 3
    if(fscanf(fparams, "%s", &params->uge_in_file) != 1){printf("ERROR reading in uge out file name\n");}
    // line 4
    if(fscanf(fparams, "%s", &params->uge_out_file) != 1){printf("ERROR reading in uge out file name\n");}
    // line 5
    if(fscanf(fparams, "%s", &params->aper_file) != 1){printf("ERROR reading in aperture file name\n");}
    // line 6
    if(fscanf(fparams, "%d", &params->cell_flag)!=1){printf("ERROR reading in cell flag\n");}
}
...
int main(int argc, char *args[]){
	...
	if(params->cell_flag > 0){printf("Using Cell Based Aperture \n");}
