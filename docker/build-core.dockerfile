...
RUN ["apt-get","install","-y","build-essential","gfortran","cmake","git",\
        "python","wget","libz-dev","m4","bison","python3",\
        "python3-pip","python3-tk","vim","curl","pkg-config","openssh-client",\
        "openssh-server","valgrind","nano","emacs"]

RUN ["pip3","install","setuptools","numpy","h5py","matplotlib","scipy","networkx",\
    "rich","pyvtk","fpdf","rich","seaborn","mplstereonet"]
...
RUN ["make","release"]
WORKDIR $APP_PATH
RUN ["mv","LaGriT/src/lagrit","bin/lagrit"]
RUN ["rm","-Rf","LaGriT/"]
