...
std::vector<Point> discretizeLineOfIntersection(double *pt1, double *pt2, double dist) {
    ...
    double v[3] = {pt2[0] - pt1[0], pt2[1] - pt1[1], pt2[2] - pt1[2]};
    double p[3] = {pt1[0], pt1[1], pt1[2]};
...
double *fisherDistribution(double angleOne, double angleTwo, double kappa, std::mt19937_64 &generator) {
    ...
    if (!(std::abs(xProd[0]) <= eps && std::abs(xProd[1]) <= eps && std::abs(xProd[2]) <= eps)) {
        ...
        double v[9] = {0, -xProd[2], xProd[1],
                       xProd[2], 0, -xProd[0],
                       -xProd[1], xProd[0], 0};
        double scalar = (1.0f - cosA) / (sinA * sinA);
        double vSquared[9];
        // compute v² = v * v
        for (int ii = 0; ii < 3; ++ii) {
            for (int jj = 0; jj < 3; ++jj) {
                vSquared[3*ii+jj] = (v[3*ii+0]*v[0*3+jj] +
                                     v[3*ii+1]*v[1*3+jj] +
                                     v[3*ii+2]*v[2*3+jj]) * scalar;
            }
        }
        // R = I + v + v²
                            ...
                            std::mt19937_64 &generator)
{
    ...
    if (!(fabs(xProd[0])<=eps && fabs(xProd[1])<=eps && fabs(xProd[2])<=eps)) {
        ...
        for (int i=0;i<3;i++) for(int j=0;j<3;j++){
            v2[3*i+j] = (v[3*i+0]*v[0*3+j] + v[3*i+1]*v[1*3+j] + v[3*i+2]*v[2*3+j]) * scalar;
        }
...
void generateTheta(float * &thetaArray, float aspectRatio, int nPoints) {
    ...
    for (int i = 1; i < nPoints; i++) {
        ...
        thetaArray[i] = thetaArray[i - 1]
                        + 0.5 * del * std::pow(std::sqrt(std::pow(b * std::cos(tmp), 2)
                        + std::pow(a * std::sin(tmp), 2)), -1)
                        + 0.5 * f_tmp;
    }
}
