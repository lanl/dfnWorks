...
def test(outputPath):
	...
	test_folder = outputPath + '/Intersection_Shortening_Tests'
	os.makedirs(test_folder)

	h =  0.25

	list = [.5-(h/2), .5-(0.9*(h/2)), .5-(.08*(h/2))]
	#Key =  [       0,              0,              0] 
	#key: 0 = test should pass, 1 = test should fail
	
	print "\n******* Running Intersection Shortening Tests: *******"
	print   "******************************************************\n"
	
	for i in range(len(list)):
        
		test_name = 'Intersection_Shortening_Test_%d' % i
		...
		input_filename = test_subFolder + 'input_' + test_name + 'Rects_%0.5f.dat' % list[i]
		...
		print "Must inspect mesh to determine success or failure of the shortening intersection algorithm (shrinkIntersection() function in computationalGeometry.cpp)\n"
