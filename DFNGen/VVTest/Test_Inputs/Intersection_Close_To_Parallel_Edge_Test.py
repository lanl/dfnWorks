...
def test(outputPath):
	...
	test_folder = outputPath + '/Close_Intersection_to_Parallel_Edge_Tests'
	os.makedirs(test_folder)

	h =  0.25

	list = [0.5, 1-h/2, 1-(.99*h/2), 1.0]
	Key =  [  0,     0,           1,   1] #last test might pass after FRAM is updated
	#key: 0 = test should pass, 1 = test should fail
	
	print "\n***** Running Close Intersection to Parallel Edge Tests: *****"
	print   "**************************************************************\n"
	
	for i in range(len(list)):
        
		test_name = 'closeIntersectionToParallelEdge_%d' % i
		test_subFolder = test_folder + "/" + test_name + "/"
		os.makedirs(test_subFolder)
		
		main_filename =  test_subFolder + "input_" + test_name + "_%0.5f.dat" % list[i]
		input_filename = test_subFolder + 'input_closeIntersectionToParallelEdgeRects_%0.5f.dat' % list[i]
