...
def test(outputPath):
	...
	test_folder = outputPath + '/Close_Intersection_to_Intersection_Tests'
	os.makedirs(test_folder)

	h =  0.25
	
	transX = [0, 1, 1+h, 1+(.99*h),  h, h-.99*h] 
	transY = [0, 0,   0,         0, .5, 	 .5]
	normX  = [0, 0,   0,         0,  1,       1]
	normY  = [1, 1,   1,         1,  0,       0] 		
	Key    = [0, 1,   0,         1,  0,       1] 
	...
	print "\n*  Running Close Intersection to Intersection Tests: *"
	print   "******************************************************\n"
	
	for i in range(len(transX)):
		 
		test_name = 'Close_Intersection_to_Intersection_Test_%d' % i 
		test_subFolder = test_folder + '/' + test_name + '/'
		os.makedirs(test_subFolder)
		
		main_filename =  test_subFolder + "input_" + test_name + "_%d.dat" % i
		input_filename = test_subFolder + 'input_Close_Intersection_to_Intersection_Rects_%d.dat' % i
		...
		f.write(input_fracture_file%(transX[i], transY[i], normX[i], normY[i]))
		f.close()
