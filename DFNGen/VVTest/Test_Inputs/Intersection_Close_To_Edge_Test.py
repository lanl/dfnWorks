...
def test(outputPath):
	...
	test_folder = outputPath + '/Close_End_Pt_To_Edge_Tests'
	os.makedirs(test_folder)

	h =  0.25

	list = [0, .5, .5-(h/2), .5-(0.9*(h/2))]
	Key =  [0, 0,       0,        1] 
	#key: 0 = test should pass, 1 = test should fail
	
	print "\n******* Running Close End Point to Edge Tests: *******"
	print   "******************************************************\n"
	
	for i in range(len(list)):
        
		test_name = 'closeEndPtToEdge_%d' % i
		...
		input_filename = test_subFolder + 'input_closeEndPtToEdgeRects_%0.5f.dat' % list[i]
