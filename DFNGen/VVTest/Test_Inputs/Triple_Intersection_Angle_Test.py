...
#Curent DFNGen triple intersection angle acceptance limit is: 47 <= angle <= 133 degrees

	#create subfolder for this test
	test_folder = outputPath + '/Triple_Intersection_Angle_Tests'
	os.makedirs(test_folder)

	h =  0.25

	listx = [0.0, 1, -1, .68199836006, .68199836006, -.68199836006, .73135370161, .8 ]
	listy = [1.0, 1, -1, .73135370161, -.73135370161, .73135370161, -.68199836006, -.871] 
	Key =   [  0, 1,  1,            0,             0,            0,             1,    0] 
	#key: 0 = test should pass, 1 = test should fail
	
	print "\n****** Running Triple Intersection Angle Tests: ******"
	print   "******************************************************\n"
	
	for i in range(len(listx)):
		
		test_name = 'TripleAngleTest_%d' % i 
		test_subFolder = test_folder + "/" + test_name + "/"
		os.makedirs(test_subFolder)
		
		
		main_filename =  test_subFolder + "input_" + test_name + "_%0.5f_%0.5f"% (listx[i], listy[i]) + ".dat" 
		input_filename = test_subFolder + 'input_TripleAngleTest_Rects_%0.5f_%0.5f.dat' % (listx[i], listy[i])
		...
		f.write(input_fracture_file%(listx[i], listy[i]))
		f.close()
		...
		if (exitVal == 0 and Key[i] == 0) or (exitVal != 0 and Key[i] == 1):
			...
			print "Test Failed (exit value = %d, Key = %d)"%(exitVal, Key[i]) + "\n"
