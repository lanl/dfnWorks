...
int InitPos()
{
    ...
    // output and check if error: if identified first and last node do not belong to the same fracture
    //  for (i=0; i<numbf; i++)
    //    {
    //    printf("%d  first node %d first ind %d last node %d last ind %d fracture %d\n", i+1, firstnode[i], firstind[i], lastnode[i], lastind[i], node[firstnode[i]-1].fracture[0]);
    //     if (node[firstnode[i]-1].fracture[0]!=node[lastnode[i]-1].fracture[0])
    //   printf("NOOO! %d %d \n", node[firstnode[i]-1].fracture[0],node[lastnode[i]-1].fracture[0]);
    //    }
...

{
    ...
    for (j=first_ind; j<=last_ind; j++)
    {
        ...
        for (jj=0; jj<pf; jj++)
        {
            ...
            //  if (particle[k_current].fracture==13)
            //      printf("os %lf %lf %lf  %lf \n", particle[k_current].position[0], particle[k_current].position[1], eqdist_x, eqdist_y);
    ...
    // Find initial cell of the new particles
    
    //  printf(" NP  %d  %d \n", np_init, k_current);
    //printf("first index %d last index %d first node  %d last node %d \n", first_ind, last_ind, nodezonein[first_ind], nodezonein[last_ind]);
    //  for (np=np_init; np<k_current; np++)
    //   {
    //      particle3dposition=CalculatePosition3D();
    // printf("3d x %lf y %lf z %lf \n", particle3dposition.cord3[0], particle3dposition.cord3[1], particle3dposition.cord3[2]);
    //   insc=0;
    //  for (j=first_ind; j<=last_ind; j++)
    //  {
    //      printf("first %d last %d current %d node %d \n", first_ind, last_ind, j, nodezonein[j]);
    //  for (i=0; i<node[nodezonein[j]-1].numneighb; i++)
    //     {
    //                 for (k=0; k<4; k++)
    //                     if (node[nodezonein[j]-1].cells[i][k]!=0)
    //                         {
    //                          curcel=node[nodezonein[j]-1].cells[i][k];
    //                         printf("curcel %d ", curcel);
    //                        insc=InsideCell(curcel);
    //  if (insc==1)
    //   { printf("found, %d %d \n", particle[np].cell, np);
    //     break;
    ...
    //           printf("np %d particle cell %d  in node %d fract %d \n", np, particle[np].cell, nodezonein[j], particle[np].fracture);
    
    //      else
    //   printf("%d find cell: particle cell %d  in node %d \n", np, particle[np].cell, nodezonein[j]);
    
    //   }
    // printf("%5.12e pf %d\n", sumfluxfract, (int)(sumfluxfract/weight_p));
    return k_current;
}
////////////////////////////////////////////////////////////////////////////
