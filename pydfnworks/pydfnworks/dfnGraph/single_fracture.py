...
def sudicky_solution(params, t_min, t_max, num_pts=100):
    ...
    c0 = 1
    w = (params["porosity"] * np.sqrt(params["mdiff"]) *
         params["frac_length"]) / ((params["b"] / 2) * params["velocity"])
    ...
    sudicky_laplace = lambda s: ((c0**2) / s) * mp.exp(-(s * z) / v) * mp.exp(
        -w * mp.sqrt(s) * mp.tanh(sig * mp.sqrt(s)))

    for i, t in enumerate(times):
        solution[i] = mp.invertlaplace(sudicky_laplace,
                                       t,
                                       method='deHoog',
                                       dps=10,
                                       degree=18)
...
def get_fracture_segments(fracture_length,
                          b,
                          velocity,
                          fracture_spacing,
                          mdiff,
    ...
    segment_length = (
        (b * np.sqrt(transfer_time)) /
        (phi * np.sqrt(mdiff))) * special.erfcinv(1.0 - plim) * velocity
...
def single_particle(trans_prob, fracture_params):
    ...
    for i in range(fracture_params["num_segments"]):
        ...
        t_diff_seg = t_diff_unlimited(fracture_params['a'],
                                      fracture_params["segment_time"], xi)
        ...
        if limited_probability > 0:
            ...
            if not quiet:
                print(f"Transition probability {limited_probability}")
...
fracture_params["a"] = (params["porosity"] *
                        np.sqrt(params["mdiff"])) / fracture_params['b']
fracture_params["advect_time"] = fracture_params[
    "frac_length"] / fracture_params["velocity"]
fracture_params["segment_length"], fracture_params[
    "num_segments"] = num_segs = get_fracture_segments(
        fracture_params["frac_length"], fracture_params["b"],
        fracture_params["velocity"], params["frac_spacing"] / 2,
        params['mdiff'], params['porosity'])
fracture_params["segment_time"] = fracture_params[
    "segment_length"] / fracture_params["velocity"]
...
times, solution = sudicky_solution(params, 1e-1 * min(total_times),
                                   1e1 * max(total_times))
fig, ax = plt.subplots()
