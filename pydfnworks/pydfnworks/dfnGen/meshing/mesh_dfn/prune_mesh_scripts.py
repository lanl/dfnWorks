...
def clean_up_files_after_prune(self, dump_files = False):
      ...
      with open(self.jobname + 'poly_info.dat', 'w') as fp:
         for i in range(self.num_frac):
            fp.write('%d %d %f %f %f %d %f %f %d\n' %
               (i + 1, self.poly_info[i, 1], self.poly_info[i, 2], self.poly_info[i, 3],
               self.poly_info[i, 4], self.poly_info[i, 5], self.poly_info[i, 6],
 ...
 #np.genfromtxt(self.path + 'radii_Final.dat', skip_header=2)[keep_list - 1, :]
  ...
  #np.genfromtxt(self.path + 'normal_vectors.dat')[keep_list - 1, :]
      for i in range(self.num_frac):
         fout.write('%f %f %f\n' %
                     (self.normal_vect[i, 0], self.normal_vect[i, 1], self.normal_vect[i, 2]))
      fout.close()
      print("--> Complete")
